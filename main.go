...
func etag() {
	for i, v := range os.Args {
		if i == 1 {
			...
			case `help`:
				fmt.Println(`敬请期待！`)
				fmt.Println(`访问 https://github.com/ZxwyWebSite/NgConf 查看文档`)
				os.Exit(0)
			case `version`:
				fmt.Println(version)
				os.Exit(0)
...
func main() {
	...
	if ver.Value() != version {
		cfg.Section(``).Key(`version`).SetValue(version)
...
func newsite() {
	...
	main := `server
{
    # ipv6监听(默认开启)，如不需要可自行注释，https配置文件里还有一个
    listen ` + site_port + `;
    listen [::]:` + site_port + `;
	# 站点域名
	...
	php := `location ~ [^/]\.php(/|$)
{
	...
	fmt.Println("\n生成完毕，请到站点管理启用站点。")
	fmt.Println("\n配置目录：" + sitedir)
	fmt.Println(`如php程序有权限问题请执行 chown -R www-data:www-data ` + site_path)
	os.Exit(0)
}
...
func setsite() {
	...
	// 获取文件夹数量，创建动态数组，解决上限20个站点问题
	var sitenum int
	for n := range files {
		sitenum = n + 1
	}
	sitename := make([]string, sitenum)
	siteactive := make([]bool, sitenum)
